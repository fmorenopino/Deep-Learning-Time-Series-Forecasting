...
def evaluate(train_seq,enc_len,dec_len,series_id,scaling_factor,encoder,pred_samples):
    with torch.no_grad():
        batch_size = train_seq.size(0)
        Pred_series = torch.zeros(pred_samples,dec_len,batch_size,1,device=device)
        for i in range(dec_len):
            mu, sigma = encoder(series_id,train_seq[:,:(enc_len+1+i),:])
            mu = mu[:,-1,:]
            sigma = sigma[:,-1,:]
...
def evaluateIters(test_loader,enc_len,dec_len,encoder,pred_samples): #Only used for testing
    predictions = []
    encoder.eval()
    for id, data in enumerate(test_loader):
        ...
        pred = evaluate(train_seq,enc_len,dec_len,series_id,scaling_factor,encoder,pred_samples)
...
def ND_Metrics(predictions,test_loader):
    ...
    for idx, data in enumerate(test_loader):
        _,gt,_, _ = data  # (batch_size,seq, value) --> (seq,batch_size,value)
...
def Rou_Risk(rou,predictions,test_loader,pred_samples):
    ...
    for idx, data in enumerate(test_loader):
        ...
        numerator += 2*(torch.sum(rou*abs_diff[gt>rou_pred])+torch.sum((1-rou)*abs_diff[gt<=rou_pred])).item()
