...
parser.add_argument('--data-folder', default='DeepAR/data', help='Parent dir of the dataset')
parser.add_argument('--model-name', default='DeepAR/experiments/base_model/', help='Directory containing params.json')
...
def train(model: nn.Module,
    ...
    for i, (train_batch, idx, labels_batch) in enumerate(tqdm(train_loader)):
        ...
        loss_epoch[i] = loss
        if i % 1000 == 0: #Checking performance on the middle of training
...
def train_and_evaluate(model: nn.Module,
    ...
    for epoch in range(params.num_epochs):
        ...
        loss_summary[epoch * train_len:(epoch + 1) * train_len] = train(model, optimizer, loss_fn, train_loader,
                                                                        test_loader, params, epoch) #Here, instead of test_loader, we should have a val_loader to avoid model selection on test
        test_metrics = evaluate(model, loss_fn, test_loader, params, epoch, sample=args.sampling)
...
if __name__ == '__main__':
    ...
    #params.plot_dir = os.path.join(model_dir, 'figures_max_samples=100')
    ...
    #sampler = WeightedSampler(data_dir, args.dataset, replacement=True, max_samples=100)
