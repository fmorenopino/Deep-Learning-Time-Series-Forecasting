...
parser.add_argument('--path', default='ConvTrans/data/', type=str,
                    help='Time series data path')
parser.add_argument('--outdir', default='ConvTrans/runs', type=str,
...
parser.add_argument('--train-ins-num', default=500000, type=int,#500000
...
parser.add_argument('--pred_days', default=24, type=int,#7
                    help='How many days to predict after the train_end date')
parser.add_argument('--seed', default=0, type=int,
                    help='set random seed')
parser.add_argument('--enc_len', default=168, type=int,#168
                    help='The lengh of training range (encoder part)')
parser.add_argument('--dec_len', default=24, type=int,#24
...
parser.add_argument("--dataset", default='wind',type=str,
                    help="Dataset you want to train")
parser.add_argument('--v_partition',default=0.3, type=float,
                    help='validation_partition')
parser.add_argument('--q_len',default=1, type=int,
                    help='kernel size for generating key-query')
parser.add_argument('--early_stop_ep',default=5, type=int,
                    help='early_stop_ep')
    ...
    for epoch in tqdm(range(epochs), desc='Epochs', unit='epoch'):
        ...
        with tqdm(train_loader, desc=f'Epoch {epoch+1}/{epochs}', unit='batch') as tepoch:
            for step, data in enumerate(tepoch):
                ...
                tepoch.set_postfix(loss=tr_loss / (step + 1))  # Optional: display current loss
...
def main():
    ...
    args.path = args.path + args.dataset + '.csv'
